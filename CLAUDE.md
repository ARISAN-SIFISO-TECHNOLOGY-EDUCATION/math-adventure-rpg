...
- **Framework core** (`src/exam-studio/`): the **content-free, reusable** exam-prep package (in-repo; see `docs/adr/001-exam-studio-package.md`). `types.ts` (the `Problem` contract + `LevelGenerator`/`TopicLevels`), `helpers.ts` (`makeOptions`, `fromCases`, `randInt`, `shuffle`, `gcd`, `simplify`, `expOptions`, `comb`, `perm`, `uid`), `progress.ts` (mastery engine). Public API via the barrel `src/exam-studio/index.ts`. **Dependency rule: `exam-studio ← senior (content) ← App`** — the framework imports no app content/UI (its one host seam is `src/lib/safeStorage`). Consumers import from the barrel (`from '../../exam-studio'`); `mathEngine.ts` imports `types.ts`/`helpers.ts` *directly* with `.ts` extensions so the Node smoke test skips the localStorage path.
- **Math (content)** (`src/senior/mathEngine.ts`): `TOPIC_LEVELS` maps `'age{N}-{topic}'` → `{ levelNumber: generator }`. Each generator returns the IGCSE `Problem` type: `question, correctAnswer, options:[string×4], marks, workingSteps[], hints[], calculatorAllowed, commonMistake, examTip`. `makeOptions(correct, wrong[])` guarantees 4 distinct options (de-dupe + numeric-neighbour padding). `fromCases(CaseDef[])` builds compact conceptual generators; `comb`/`perm` are shared helpers. (These helpers now live in `exam-studio`.)
